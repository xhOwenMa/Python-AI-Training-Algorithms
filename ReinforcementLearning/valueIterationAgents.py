    ...
    def runValueIteration(self):
        ...
        for iteration in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                iteration_values[state] = max([self.getQValue(state, action) for action in actions])
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            ...
            actions = self.mdp.getPossibleActions(state)
            max_qValue = max([self.getQValue(state, action) for action in actions])
        ...
        for iteration in range(self.iterations):
            if not PQueue.isEmpty():
                ...
                for p in predecessors[state]:
                    ...
                    high_qValue = max([self.getQValue(p, action) for action in self.mdp.getPossibleActions(p)])
                    diff = abs(self.values[p] - high_qValue)
